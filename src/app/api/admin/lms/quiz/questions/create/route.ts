...
export async function POST(req: Request) {
  ...
  if (!body?.quiz_id || !body?.question?.trim() || !body?.option_a?.trim() || !body?.option_b?.trim() || !body?.correct_option?.trim()) {
    return NextResponse.json({ ok: false, message: "Missing required question fields." }, { status: 400 });
  }

  const { error } = await supabaseServer.from("sod_quiz_questions").insert([
    {
