...
export default function AdminLMSExamSubmissionsPage() {
  ...
  async function load() {
    ...
    const res = await fetch("/api/admin/lms/exam/submissions", {
      cache: "no-store",
    });
    const data = await res.json().catch(() => null);

    if (!res.ok || !data?.ok) {
      setMsg(data?.message || "Failed to load final exam submissions");
      setLoading(false);
      return;
    }
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((row) => {
      ...
      const exam = row.sod_exams?.title || "";

      return (
        name.toLowerCase().includes(text) ||
        email.toLowerCase().includes(text) ||
        exam.toLowerCase().includes(text)
      );
    });
  }, [rows, q]);
  ...
  return (
    <main>
      <PageHero
        title="Admin — Final Exam Submissions"
        subtitle="View final exam scores and student submissions."
      />
          ...
          <div className="overflow-hidden rounded-2xl border">
            <div className="bg-slate-50 px-5 py-4">
              <p className="font-semibold text-slate-900">
                Final Exam Submissions ({filtered.length})
              </p>
            </div>
            ...
            {loading ? (
              ...
              <div className="p-8 text-slate-600">No final exam submissions yet.</div>
            ) : (
                ...
                <table className="min-w-full text-left">
                  <thead className="border-b bg-white">
                    <tr className="text-sm text-slate-600">
                      ...
                      <th className="px-5 py-4">Final Exam</th>
                  ...
                  <tbody>
                    {filtered.map((row) => (
                      <tr key={row.id} className="border-b text-sm text-slate-700">
                        ...
                        <td className="px-5 py-4">{row.sod_exams?.title || "—"}</td>
